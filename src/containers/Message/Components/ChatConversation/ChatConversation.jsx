import { 
  ...
  const StyledListItemButton = styled(ListItemButton)(({ theme, selected }) => ({
    ...
    // Normal state
    background: selected 
      ? 'linear-gradient(135deg, rgba(186, 104, 200, 0.2) 0%, rgba(255, 105, 180, 0.2) 100%)' 
      : 'transparent',
    
    // Hover state
    '&:hover': {
      background: selected
        ? 'linear-gradient(135deg, rgba(186, 104, 200, 0.25) 0%, rgba(255, 105, 180, 0.25) 100%)'
        : 'linear-gradient(135deg, rgba(186, 104, 200, 0.1) 0%, rgba(255, 105, 180, 0.1) 100%)',
    ...
    '&::after': {
      ...
      width: '3px',
      background: selected 
        ? 'linear-gradient(180deg, #ff69b4, #ba68c8)' 
        : 'transparent',
  ...
  const UserName = styled(Typography)(({ theme, hasNotification }) => ({
    background: hasNotification 
      ? 'linear-gradient(90deg, #ff69b4, #ba68c8)' 
        ...
        <StyledListItemText
          primary={
            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%' }}>
              ...
              {hasNotification && (
                <NotificationDot sx={{ fontSize: 10 }} />
              )}
            </Box>
          }
        />
